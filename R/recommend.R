setSimilarityValue = function(converted_user_input) {
  ...
  #setAccommodation
  for(i in 1:nrow(new_data)) {
    sim_table$nAccommodation[i] = ifelse(converted_user_input["nAccommodation"] <= new_data$nAccommodation[i], price_class1, price_class2)
  }
...
getTop5Case = function(user_input) {
  ...
  names(sim_table2) <<- c("JourneyCode", "Total", "HolidayType", "Price", "Transportation", "NumberOfPerson", "Region", "Season", "Duration", "Accommodation")
  ...
  sim_table2 <<- sim_table2[order(sim_table2["Total"], decreasing = TRUE), ]
  
  # top 5
  travelCases[sorted_sim$caseID[1:5], -2]
}

getClusteredList = function(user_input) {
  # convert user input
  print(c(user_input["Season"], user_input["Price"]))
  converted_user_input = c(convert_season_to_value(user_input["Season"]),
                           as.numeric(parse_user_input(user_input["Price"])))
  
  clusters <- function(x, centers) {
    #print(rbind(x, centers))
    dist(rbind(x, centers))
  }
  
  clusterNo <<- which.min(apply(kmeansResult[["centers"]], 1, clusters, x = converted_user_input))
  
  travelCases[kmeansResult[["cluster"]] == clusterNo, -2]
}